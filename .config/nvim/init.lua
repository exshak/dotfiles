...
local plugins = {
  ...
  -- illuminate {{{2
  {
    ...
    opts = {
      filetypes_denylist = { "checkhealth", "conf", "fugitive" },
  ...
  -- telescope {{{2
  {
    ...
    keys = function()
      ...
      return {
        ...
        { "<leader>fc", LazyVim.pick("files", { cwd = vim.fn.stdpath("config") }), desc = "Find config" },
        { "<leader>fC", LazyVim.pick("files", { cwd = vim.env.XDG_CONFIG_HOME }), desc = "Find config files" },
        ...
        { "<leader>ss", LazyVim.pick("lsp_document_symbols", { symbols = symbols }), desc = "Goto symbol" },
        { "<leader>sS", LazyVim.pick("lsp_dynamic_workspace_symbols", { symbols = symbols }), desc = "Goto symbol (work)" },
...
map("nx", "<C-s>", ":w<cr><esc>", { desc = "Save file" })
...
map("nv", "<leader>cf", function() LazyVim.format({ force = true }) end, { desc = "Format" })
map("n", "<leader>ch", ":checkhealth<cr>", { desc = "Checkhealth" })
map("n", "<leader>fn", ":enew<cr>", { desc = "New file" })
map("n", "<leader>ft", function() LazyVim.terminal(nil, { cwd = LazyVim.root() }) end, { desc = "Terminal (root)" })
map("n", "<leader>fT", function() LazyVim.terminal() end, { desc = "Terminal (cwd)" })
map("n", "<leader>gB", LazyVim.lazygit.blame_line, { desc = "Blame line" })
map("n", "<leader>gf", function() LazyVim.lazygit({ args = { "-f", vim.trim(vim.api.nvim_buf_get_name(0)) } }) end, { desc = "Lazygit (file)" })
map("n", "<leader>gg", function() LazyVim.lazygit({ cwd = LazyVim.root.git() }) end, { desc = "Lazygit (root)" })
...
map("n", "<leader>uc", function() LazyVim.toggle("conceallevel", false, { 0, vim.o.conceallevel > 0 and vim.o.conceallevel or 3 }) end, { desc = "Toggle conceal" })
...
map("n", "<leader>ul", function() LazyVim.toggle("list") end, { desc = "Toggle list" })
map("n", "<leader>ur", function() LazyVim.toggle("relativenumber") end, { desc = "Toggle relative numbers" })
