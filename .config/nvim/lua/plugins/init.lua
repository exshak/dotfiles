    ...
    opts = function()
      ...
      return {
        ...
        options = {
          ...
          disabled_filetypes = {
            statusline = { "checkhealth", "lazy", "lazyterm", "mason", "snacks_dashboard", "TelescopePrompt" },
          },
          globalstatus = true,
          section_separators = "",
          theme = "auto",
        ...
        sections = {
          ...
          lualine_x = {
            ...
            LazyVim.lualine.status(LazyVim.config.icons.kinds.Copilot, function() local clients = package.loaded["copilot"] and LazyVim.lsp.get_clients({ name = "copilot", bufnr = 0 }) or {} if #clients > 0 then local status = require("copilot.api").status.data.status return (status == "InProgress" and "pending") or (status == "Warning" and "error") or "ok" end end),
      ...
      dashboard = {
        preset = {
          ...
          keys = {
            ...
            { icon = " ", key = "f", desc = "Find", action = ":Telescope fd" },
            { icon = " ", key = "g", desc = "Grep", action = ":lua Snacks.dashboard.pick('live_grep')" },
            { icon = " ", key = "p", desc = "Project", action = ":Telescope projects" },
            { icon = " ", key = "r", desc = "Recent", action = ":Telescope frecency" },
